...
"""
    Scrapes the Italian newspaper "openmigration"
...
numwords = 100
newspaper = 'openmigration'

# Define the folder path containing the HTML files and the output CSV file
folder_path = r"C:/Users/MariaΙoannaTzortzi/OneDrive - ITML/AFRO_EQUALITY/New_Attempt/webpages_html/Italian/openmigration"
...
for file_name in file_list:
    ...
    if file_name.endswith(".html"):  # Only process HTML files
        ...
        content_blocks = soup.find("main", class_="content")

        if content_blocks:
            print(f'✅ Found content grid')

            # Try other possible article content containers
            content_section = content_blocks.find("div", class_="entry-content") or \
                            content_blocks.find("div", class_="container")             
            if content_section:
